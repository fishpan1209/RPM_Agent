...
public class Agent {
	...
	public String Solve(VisualRavensProblem problem) {
		/*
		 * if (!"2x1".equals(problem.getProblemType())) { throw new
		 * RuntimeException
		 * ("This agent is designed to solve only 1x2 type Ravens problems."); }
		 */
        ...
        VisualRavensFigure VisualFigA = problem.getFigures().get("A");
        RavensFigure figA = new RavensFigure(VisualFigA);
    	
        VisualRavensFigure VisualFigB = problem.getFigures().get("B");
        RavensFigure figB = new RavensFigure(VisualFigB);
        
        
        VisualRavensFigure VisualFigC = problem.getFigures().get("C");
        RavensFigure figC = new RavensFigure(VisualFigC);
       
       
		FigureDiff demoFigDiffAB = new FigureDiff(figA, figB);
		FigureDiff demoFigDiffAC = new FigureDiff(figA, figC);
		
		//test
		/**
		System.out.println(problem.getName());
		for (ObjectDiff objDiff1 : demoFigDiffAB.getDifferences()) {
			
			for (AttributeDiff attrDiff1 : objDiff1.getDifferences().values()) {
				System.out.println(attrDiff1.getFirst().getName()+":"+attrDiff1.getFirst().getValue()+
						"//"+attrDiff1.getSecond().getName()+":"+attrDiff1.getSecond().getValue());
				
			}
			System.out.println("__________________");
		}**/
		// String currentBestAnswer = null;
		String currentBestAnswer = "6"; // default answer

		double bestAnswerFitness = 0;
		
		//couldn't read 04,11, default answer
		if (figA.getObjects().size()==0) {
			currentBestAnswer = "4";
		}
	
		
		for (int i = 1; i <= 6; ++i) {
			String candidateLabel = Integer.toString(i);
			RavensFigure answerCandidate = new RavensFigure(problem.getFigures().get(candidateLabel));
		
			
			if ("2x1 (Image)".equals(problem.getProblemType())) {
				
				double candidateFitness = demoFigDiffAB.similarity(
						demoFigDiffAB, new FigureDiff(figC, answerCandidate));
				System.out.println(candidateFitness);
               //System.out.println(candidateFitness);
				if (candidateFitness > bestAnswerFitness) {
					currentBestAnswer = candidateLabel;
					bestAnswerFitness = candidateFitness;
				}

			}

			else if ("2x2 (Image)".equals(problem.getProblemType())) {
/**
				double candidateFitness = demoFigDiffAB.similarity2x2(
						demoFigDiffAB, demoFigDiffAC, new FigureDiff(figC,
								answerCandidate), new FigureDiff(figB,
								answerCandidate));
				...
				currentBestAnswer = solve2x2(problem);
			}

			else {
				throw new RuntimeException(
						"This agent is not able to solve current problem sets");
			}
		...
		/**
            	...
            	Mat matA = Highgui.imread(VisualFigA.getPath(), 0);
            	Mat matB = Highgui.imread(VisualFigB.getPath(), 0);
            	Mat matC = Highgui.imread(VisualFigC.getPath(), 0);
          
            	
            	ArrayList<HashMap<String, String>> figAList = analyzeImage(VisualFigA.getPath());
            	ArrayList<HashMap<String, String>> figBList = analyzeImage(VisualFigB.getPath());
            	ArrayList<HashMap<String, String>> figCList = analyzeImage(VisualFigC.getPath());
